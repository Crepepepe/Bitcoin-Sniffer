...
public class IPv6 {
    ...
    private final String v4Regex = "^([0-9]{1,3}\\.){3}[0-9]{1,3}?$";
    private final String v6Regex = "^s*((([0-9A-Fa-f]{1,4}:){7}([0-9A-Fa-f]{1,4}|:))|(([0-9A-Fa-f]{1,4}:){6}(:[0" +
    ...
    private final long[] pow = {16777216, 65536, 256, 1};

    private String ip;

    public IPv6 (String ip) throws InvalidIpAddressException {
        if(ip.matches(v4Regex)) {
            ...
            throw new InvalidIpAddressException("Wrong ip address: " + ip);
        }
    }
